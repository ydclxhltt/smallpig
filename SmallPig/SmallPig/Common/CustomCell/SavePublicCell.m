...
- (void)addHouseImageView
{
    ...
    _houseImageView = [CreateViewTool createImageViewWithFrame:CGRectMake(start_x, (SAVE_LIST_HEIGHT - defaultImage.size.height/2)/2, defaultImage.size.width/2, defaultImage.size.height/2) placeholderImage:defaultImage];
...
- (void)setCellImageWithUrl:(NSString *)imageUrl titleText:(NSString *)title localText:(NSString *)local parkText:(NSString *)park priceText:(NSString *)price typeText:(NSString *)type sizeText:(NSString *)size
{
    ...
    NSArray *array = @[local,park,price,type,size];

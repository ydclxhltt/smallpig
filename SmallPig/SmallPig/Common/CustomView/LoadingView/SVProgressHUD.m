...
- (void)showWithStatus:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show closeSel:(SEL)closeSel target:(id)target;
...
+ (void)showWithStatus:(NSString*)status maskType:(SVProgressHUDMaskType)maskType closeSel:(SEL)closeSel target:(id)target {
    [[SVProgressHUD sharedView] showWithStatus:status maskType:maskType networkIndicator:NO closeSel:closeSel target:target];
}
...
- (void)setStatus:(NSString *)string {
    ...
    self.btnClose.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds)) - 14, 14);
...
- (void)showWithStatus:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show closeSel:(SEL)closeSel target:(id)target
{
    [self showWithStatus:string maskType:hudMaskType networkIndicator:show];
    [self btnClose].hidden = NO;
    NSLog(@"alltarget = %@", [btnClose allTargets]);
    for (id target in [btnClose allTargets]) {
        [[self btnClose] removeTarget:target action:NULL forControlEvents:UIControlEventTouchUpInside];
    }
    [[self btnClose] addTarget:target action:closeSel forControlEvents:UIControlEventTouchUpInside];
...
- (UIView *)hudView {
    if(!hudView) {
		...
		hudView.backgroundColor = [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.8];
...
- (UIButton*)btnClose
{
    if (btnClose == nil) {
        ...
        btnClose.frame = CGRectMake(0, 0, 28, 28);
        [btnClose setBackgroundImage:[UIImage imageNamed:@"SVProgressHUD.bundle/error.png"] forState:UIControlStateNormal];
        btnClose.hidden = YES;
    }
    if(!btnClose.superview)
        [self.hudView addSubview:btnClose];
    return btnClose;
}
